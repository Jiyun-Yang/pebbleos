...
#define W1160_AGCCTRL1_AGC_EN       (1<<7)   /* 1:en; 0:dis */
#define W1160_AGCCTRL1_MGC_EN       (0)      /* 1:en; 0:dis */
#define W1160_AGCCTRL2_SEL_MODE     (1<<2)   /* must be 1 */
#define W1160_AGCCTRL2_12B_MODE     (1<<3)   /* 1:12bit */
#define W1160_ALSCTRL_SAT_EN        (0<<1)   /* 1:en; 0:dis */
#define W1160_ALSCTRL_DATA_FORMAT12 (1<<2)   /* 1:12bit 0:16bit */
#define W1160_SAT_GC_CONFIG         (0x0A)
#define W1160_SLOW_IT_CONFIG1       (0x00)   /* 16.830ms */
#define W1160_SLOW_IT_CONFIG2       (0x0A)
...
void ambient_light_init(void) {
  ...
  PBL_ASSERT(rv, "Failed to get W1160 chip ID");
...
uint32_t ambient_light_get_light_level(void) {
  uint8_t result[2] = {0};
  prv_read_register(W1160_DATA1_ALS_REG, &result[1]);
  prv_read_register(W1160_DATA2_ALS_REG, &result[0]);
  uint16_t als = (((uint16_t)(result[1])) << 8) | result[0];

  uint32_t exp = als >> W1160_RESULT_EXPONENT_SHIFT;
  uint32_t mant = als & W1160_RESULT_MANTISSA_MASK;
